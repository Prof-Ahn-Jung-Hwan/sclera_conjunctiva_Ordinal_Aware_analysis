    ...
    for fold in folds:
        ...
        for idx, row in df.iterrows():
            ...
            if not os.path.exists(img_path):
                ...
                break  # fold 중단하고 다음 fold로 넘어감
            else:
                print(f"✓ Image file found: {img_path}")
            
            # Hb 값 처리 - 빈 값이나 잘못된 값이 있으면 즉시 중단
            try:
                hb_value = row['Hb']
        ...
        # Few-shot training에서는 validation 없이 training만 수행
        # dataset 만들기 - train만
        ...
        # Few-shot training을 위한 batch size 설정
        batch_size_config = {
            'eyedye': 10,    # 30개 데이터
            'ghana': 20     # 100개 데이터
        }
        
        # external_type을 batch_size_config 키로 매핑
        dataset_key = 'eyedye' if args.external_type == 'joint' else args.external_type
        train_batch_size = batch_size_config.get(dataset_key, args.train_batch_size)  # 기본값으로 fallback
        ...
        for epoch in range(args.epochs):
            ...
            # Few-shot 학습에서는 train loss를 기준으로 best model 저장
        ...
        plot_logs(
            xs,
            xs,  # Few-shot에서는 test가 없으므로 train xs 사용
            ys1,
            ys2,
            ys1,  # Few-shot에서는 test가 없으므로 train loss 사용
            ys2,  # Few-shot에서는 test가 없으므로 train mae 사용
